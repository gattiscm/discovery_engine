...
class DiscoveryModel:
    ...
    def _score_latent(self, z):
        ...
        for category in self.memory.categories():
            ...
            reconstruction = (
                self.memory.reconstruct(category)
            )
    ...
    def _learn_novelty_cutoff(self, Z):
        ...
        self.training_novelty_scores_ = (
            np.asarray(training_scores)
        )

        self.novelty_cutoff_ = (
        ...
        print()
        print(
            f"Stored novelty cutoff: "
            f"{self.novelty_cutoff_:.4f}"
        )
        print()
    ...
    def _build_encoder(self):
        ...
        if self.encoder_type == "tabular":
            ...
            from features.tabular_extractor import (
                TabularExtractor
            )

            self.extractor = TabularExtractor(
                **self.encoder_kwargs
            )
            ...
            raise ValueError(
                f"Unknown encoder: {self.encoder_type}"
            )
    ...
    def fit(
        ...
        X_scaled = (
            ...
            self.feature_space.fit_transform(
                X
            )
    ...
    def latent_contributors(
            self,
            latent_idx
    ):

        if (
                self.latent_map is None
                or
                latent_idx not in self.latent_map
        ):
            return []

        contributors = []

        for row in self.latent_map[
            latent_idx
        ]:
    ...
    def compare_to_category(
        ...
        for latent_idx in ranked[:top_k]:
            ...
            raw_delta = mean_delta[
                latent_idx
            ]

            contributors = (
                self.latent_contributors(
                    latent_idx
                )
            )
    ...
    from engine.reports.discovery_report import (
        DiscoveryReport
    )

    def auto_discovery_report(
            self,
            X,
            top_k=20
    ):

        results = self.detect_novelty(
            X
        )
    ...
    def explain(self, x):
        ...
        if not self.is_fit:
            raise ValueError(
                "DiscoveryModel must be fit before explain()."
            )
        ...
        for category in self.memory.categories():
            ...
            reconstruction = self.memory.reconstruct(
                category
            )
            ...
            latent_contrib = contrib[
                "magnitude"
            ]

            latent_direction = contrib[
                "direction"
            ]

            latent_signed = contrib[
                "signed"
            ]

            input_contrib = np.zeros(
                n_features
            )

            input_direction = np.zeros(
                n_features
            )
            ...
            if len(self.groups) > 0:
                ...
                for group_idx, group_name in enumerate(
                    ...
                    # Map each input group to a slice
                    # of latent space when possible.
                    ...
                    latent_slice = latent_contrib[
                                   start:end
                                   ]

                    direction_slice = latent_direction[
                                      start:end
                                      ]
                    ...
                    if len(latent_slice) == 0:
                        ...
                        group_strength = np.sum(
                            latent_slice
                        )

                        group_direction = np.sign(
                            np.mean(
                                direction_slice
                            )
                        )
            ...
            if len(self.groups) > 0:
                ...
                group_contrib = compute_group_contributions(
                    input_contrib,
                    groups=self.groups
                )
            ...
            report[category] = {
                ...
                "feature_contributions":
                    input_contrib.tolist(),

                "feature_direction":
                    input_direction.tolist(),

                "signed_residual":
                    latent_signed.tolist(),
